...
Artifact definitions are in two main categories: what it needs/requires (_Requirements_) and what it gives/provides (_Provisions_). _Parameters_ are additional configuration given to control execution.
...
The provisions when executing a specific [Artifact](#artifact). This includes Healthchecks, Provided [Resources](#resource), and Provided [Services](#service). Maintained by a [Project Author](#project-author) inside a [Bolt](#bolt).

#### Resource

An entity maintained by an [Artifact](#artifact) that can be required by other [Artifacts](#artifact) for their own execution.

Resources are referenced by their `project` and `name`.

Examples: a PostgreSQL database, a Redis index, etc.

`ProvidedResource` in Artifact A + `ResourceRequirement` in Artifact B = unique `Resource`

#### Service

A named network interface to communicate with an executing [Artifact](#artifact).

Services are referenced by their `project`, `artifact`, and `name`.

Artifacts may _require_ a Service to be available.
...
The parameters given to an [Artifact](#artifact) for execution in an [Environment](#environment). This is where [Services](#service) can be externalized, [Volumes](#volume) can be determined, and so on. Maintained by an [Environment Manager](#environment-manager).
...
- Launches and maintains common Provided Resources across Environments.
